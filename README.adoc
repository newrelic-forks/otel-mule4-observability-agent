...
1.  **OTEL Semantic Metrics for APM Transaction View**:
    *   The agent now correctly calculates the duration of HTTP-triggered flows and captures standard OpenTelemetry HTTP attributes (`http.request.server.duration, http.request.method`, `url.scheme`, `http.route`, `http.response.status_code` ...).
